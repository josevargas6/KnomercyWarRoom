[CmdletBinding()]
param([string]$OutFile = 'knowledge\release-program-status.json')
...
$primaryPass = $discrepancy -and $discrepancy.summary.total -gt 0 `
...
foreach ($index in 0..12) {
    ...
    $packageStatus = if ($package -eq 'P00' -and (Bool $recovery.summary.complete)) {'PASS'} elseif ($implemented) {'IN_PROGRESS'} else {'PLANNED'}
    $stages += [ordered]@{
        id = ('KWR-297-' + $package)
