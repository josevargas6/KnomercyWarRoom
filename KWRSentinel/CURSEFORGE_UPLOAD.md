...
```text
<absolute path to certified KWR-Sentinel-6.1.1-alpha.10.zip>
```
...
- File: `KWR-Sentinel-6.1.1-alpha.10.zip`
- Display name: `KWR Sentinel 6.1.1-alpha.10`
- Release type: `Alpha`
...
- Sentinel ZIP SHA-256: use the generated
  version-derived `KWR_<VERSION>_SHA256.txt` manifest next to the built artifact. The
  exact hash is intentionally not embedded here because this file is packaged
  inside the ZIP.

Do not upload:

- `KnomercyWarRoom-6.1.1-alpha.10.zip`
