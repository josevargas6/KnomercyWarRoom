...
local function buildNode(roster, definition, snapshot)
    ...
    local result, used = {}, {}
    local minimumFight = math.min(6, math.max(3, #roster - 1))
    local defenderLimit = math.min(#controlled, math.max(1, #roster - minimumFight))
...
function Assignments:Integrity(snapshot, assignments)
    ...
    local sessionKey = tostring(context.mapID or context.mapKey)
        .. (context.preview and ":preview" or ":live")
