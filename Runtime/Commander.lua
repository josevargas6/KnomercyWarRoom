...
function Commander:Compose(snapshot, prediction, assignments)
    ...
    local command = {
        ...
        decisionScore = strategy.decisionScore,
        projection = strategy.projection,
        recommendationMode = strategy.recommendationMode,
        selectedAction = KWR.Util:Copy(strategy.selectedAction),
        simulations = KWR.Util:Copy(strategy.simulations),
        responseContract = KWR.Util:Copy(strategy.responseContract),
        truth = KWR.Util:Copy(snapshot.truth),
