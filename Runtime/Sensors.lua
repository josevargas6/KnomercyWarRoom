...
local function applyFallbackPositions(result, definition)
    ...
    for _, label in ipairs(definition.locations or {}) do
        ...
        if position and (not row.x or not row.y) then
            ...
            resolveObjectiveField(row, "position", { x = row.x, y = row.y },
                "map_definition", row.observedAt or Util:Now(), row.revision or 0, {
                    source = { system = "map_definition" },
                })
        end
    end
end

local function requestScoreboard(force)
    local now = Util:Now()
    if not force
        and now - (Sensors.lastScoreRequestAt or -999) < SCOREBOARD_REQUEST_INTERVAL then
        return
    end
