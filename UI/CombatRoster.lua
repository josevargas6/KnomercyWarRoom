...
function CombatRoster:Visual(row, data, team, combat, assignment)
    ...
    if team == "ENEMY" then
        ...
        row.stateText:SetText(data.carrier and ("CARRIER"
            .. ((data.carrierStacks or 0) > 0 and (" x" .. tostring(data.carrierStacks)) or ""))
            or (kill and "KILL"
            or (active and "DEF ACTIVE"
            or (trinket and "TRINKET USED" or observed))))
        ...
        row.glow:SetShown(kill == true)
        row:SetBackdropBorderColor(KWR.Theme:Color(kill and "red"
            or (targeted and "gold" or (focused and "purple" or "border"))))
    else
...
function CombatRoster:Layout(mode)
    ...
    self.frame:SetSize(both and 598 or 300, 378)
